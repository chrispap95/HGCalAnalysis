...
{ 
   ...
   */
   ...
   while (fReader.Next()) {
     ...
     bool norm_jet        = false; // no constaints 
     bool large_neutral   = true; // jet makeup consists of a large amount of neutral hadrons/Em
     //===============================================
     ...
     for (int ijet = 0, njet = PFJetsPt.GetSize(); ijet < njet; ++ijet) {
       ...
       fill1D(v_hist,"PFtotalneutralEFraction", PFJetsrecoJetsneutralEnergyFraction[ijet] + PFJetsrecoJetsneutralEmEnergyFraction[ijet]);
     }	 
...
void ana_PFStudy(std::string rootfile="./ttbar_10_4_D30_pt25.root",TString outfile="PFD30_histos.root",int maxevents=19400)
{
  // edit 
